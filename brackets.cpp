...
Node merge(Node &obj1, Node &obj2){
	Node result = Node();
	int mini = min(obj1.open,obj2.close);
	result.open = obj1.open + obj2.open - mini;
	result.close = obj1.close + obj2.close - mini;
	result.full = mini + obj1.full + obj2.full;
	return result;
}

void build(int index, int low, int high, string &str, vector<Node> &array){
	if(low == high){
		array[index]=Node(str[low] == '(', str[low] == ')',0);
		return;
	}
	int middle = low + (high-low)/2;
	build(2*index+1,low,middle,str,array);
	build(2*index+2,middle+1,high,str,array);
	array[index] = merge(array[2*index+1], array[2*index+2]);
}


Node query(int index, int low, int high, int l, int r, vector<Node> &array){
	if(high < l or r < low){
		return Node();
	}
	if(low >= l and high <= r){
	...
	int middle = low + (high-low)/2;
	Node left = query(2*index+1,low,middle,l,r,array);
	Node right = query(2*index+2,middle+1,high,l,r,array);
...
int main(){
	...
	cin>>str;
	int length = str.size();
	vector<Node> array(4*length+1,Node());
	build(0,0,length-1,str,array);
	int m;
	cin>>m;
	while(m--){
		int l,r;
		cin>>l>>r;
		Node result = query(0,0,length-1,l-1,r-1,array);
